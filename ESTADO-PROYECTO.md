...
- **Conexión a la base preparada para serverless**: `datasource db` en `schema.prisma` ahora separa `url` (conexión pooled, la usa la app) de `directUrl` (conexión directa, solo para migraciones) — necesario en Vercel porque cada invocación puede abrir su propia conexión a Postgres y sin pooler se agotan rápido con tráfico real. Falta cargar `DATABASE_URL` (pooled) y `DIRECT_URL` (directa) con las URLs que da Neon/Supabase.
- **Consultas sin límite corregidas**: `GET /api/chats` ahora trae máximo 50 conversaciones y `GET /api/chats/[id]` máximo los últimos 200 mensajes — antes ambas traían el historial completo sin tope, lo que iba a degradarse con el uso real.
